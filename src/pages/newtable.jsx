...
const headCells = [
  ...
  { id: 'snp_id', numeric: false, disablePadding: false, label: 'SNP ID' }, 
  { id: 'p_value', numeric: true, disablePadding: false, label: 'P-value' },
  ...
  return (
    ...
    >
      ...
      {numSelected > 0 ? (
        ...
        <Tooltip title="Add">
          <IconButton aria-label="Add" onClick={() => { console.log(selected)
            //selected.map((n) => console.log(n.props) )
             }}> 
            <PlusIcon/> 
          </IconButton>
        </Tooltip>
...
const useStyles = makeStyles((theme) => ({
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = rows.map((n) => (n.snp_id)); 
          ...
          >
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  const isItemSelected = isSelected(row.snp_id);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.snp_id )}
                      role="checkbox"
                    ...
                    >
                      ...
                      <TableCell align="left">{row.snp_id}</TableCell>
